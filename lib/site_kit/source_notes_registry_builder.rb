  ...
  class SourceNotesRegistryBuilder
    ...
    def record
...

      {
        ...
        "modules" => languages.flat_map { |language| language.fetch("modules").map { |module_record| homepage_module_record(language, module_record) } },
        "languages" => languages
      }
    end
    ...
    def language_records
      @language_records ||= source_catalog.languages.map do |language|
...

        {
          ...
          "modules" => language.modules.map { |module_definition| module_builder.build(module_definition: module_definition, language_context: language_context) }
        }
      end
    end
