...
module SiteKit
  ...
  module Core
    module Helpers
      ...
      def slugify(value)
        value.to_s.downcase.gsub(/[^a-z0-9_-]+/, '-').squeeze('-').gsub(/\A-|-+\z/, '')
      end

      def human_label(value)
        value.tr('_', ' ').split.map { |part| part[0] ? part[0].upcase + part[1..] : part }.join(' ')
      end
      ...
      def rewrite_markdown_images(markdown, base_directory, source_url_base, source_root: repo_root)
        ...
        rewritten = markdown.to_s.gsub(MARKDOWN_IMAGE_PATTERN) do |match_text|
          ...
          next match_text if raw_reference.empty? || raw_reference.start_with?('http://',
                                                                               'https://', '/')

          source_path = File.expand_path(raw_reference, base_directory)
          next match_text unless File.exist?(source_path)

          relative_asset_path = relative_path(source_root, source_path)
      ...
      def index_by(values)
        values.to_h { |value| [yield(value), value] }
      end
    ...
    class Schema
      def initialize(record, context)
        @record = Helpers.ensure_hash(record, context)
        @context = context
      end

      def required_hash(key) = Helpers.ensure_hash(record.fetch(key), field(key))
      def required_array(key) = Helpers.ensure_array(record.fetch(key), field(key))
      def required_array_of_strings(key) = Helpers.ensure_array_of_strings(record.fetch(key), field(key))
    ...
    class ResourcePaths
      ...
      def root = join
      def path(*segments) = join(*segments)
      def embed(*segments) = join(*segments, 'embed')
    ...
    module CodeEntry
      ...
      def normalize(raw, context:)
        ...
        entry = {
          ...
          'variant' => Helpers.ensure_string(record.fetch('variant'), "#{context}.variant"),
          'variant_label' => Helpers.ensure_string(record.fetch('variant_label'), "#{context}.variant_label"),
